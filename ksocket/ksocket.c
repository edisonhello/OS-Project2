...
ksocket_t kaccept(ksocket_t socket, struct sockaddr *address,
                  int *address_len) {
  struct socket *sk;
...
ssize_t krecvfrom(ksocket_t socket, void *buffer, size_t length, int flags,
...
int ksetsockopt(ksocket_t socket, int level, int optname, void *optval,
                int optlen) {
  struct socket *sk;
  int ret;
#ifndef KSOCKET_ADDR_SAFE
...
int kgetsockopt(ksocket_t socket, int level, int optname, void *optval,
                int *optlen) {
  /*	struct socket *sk;
          int ret;
          mm_segment_t old_fs;
...
static void ksocket_exit(void) { printk("ksocket exit\n"); }
